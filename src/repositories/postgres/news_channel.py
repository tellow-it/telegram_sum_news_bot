...
class NewsChannelRepository:
    ...
    async def check_if_news_channel_exists(telegram_url: str):
        async with async_session() as session:
            result = await session.execute(select(NewsChannel).where(NewsChannel.telegram_url == telegram_url))
    ...
    async def delete_news_channel(telegram_url: str):
        async with async_session() as session:
            news_channel = await NewsChannelRepository.get_news_channel(telegram_url=telegram_url)
            if not news_channel:
                raise Exception(
